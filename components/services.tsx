        ...
        <div className="grid gap-6 lg:grid-cols-3">
          {services.map((service, index) => (
            <motion.article key={service.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} viewport={{ once: true }} className={`relative flex flex-col overflow-visible rounded-2xl border bg-white/5 ${service.title === 'Full Detail' ? 'border-blue-400 ring-2 ring-blue-400/70 shadow-lg shadow-blue-500/20' : 'border-white/20'}`}>
              {service.title === 'Full Detail' && <span className="absolute right-5 top-0 z-10 -translate-y-1/2 rounded-full border border-blue-300/50 bg-blue-500 px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-white shadow-sm">{flagshipBadge}</span>}<div className="relative h-52 overflow-hidden rounded-t-2xl"><img src={service.image} alt={service.title} className="h-full w-full object-cover" /></div>
