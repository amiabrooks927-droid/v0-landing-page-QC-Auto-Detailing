...
const services = [
  { title: 'Full Detail', description: 'A complete interior and exterior reset for drivers who want every surface refreshed.', image: 'https://hebbkx1anhila5yf.public.blob.vercel-storage.com/81e4f22d507b3575386f46807e703c3c-GH4OKz8TGvzdUZEMk0PxMQbStBOl1v.jpg', items: ['Thorough interior cleaning', 'Exterior hand wash', 'Wheels and tires', 'Spray ceramic protection'] },
  { title: 'Interior Detail', description: 'A focused deep clean that brings comfort, clarity, and a fresh feel back to your cabin.', image: 'https://hebbkx1anhila5yf.public.blob.vercel-storage.com/adecbf1557ef5ae55633abd1c63ab271-nW6vNIBgSDhvncTMmHJbWtHEDv7FRc.jpg', items: ['Vacuum and surface cleaning', 'Seats, carpets, and mats', 'Dash and console detailing', 'Interior protection'] },
  { title: 'Exterior Detail', description: 'A careful exterior service that restores gloss and leaves your paint smooth and protected.', image: 'https://hebbkx1anhila5yf.public.blob.vercel-storage.com/IMG_9101-RPL5I3KJwhydAFNNcm4fcijUZb1DHd.jpg', items: ['Foam wash and hand wash', 'Wheels and tires', 'Paint decontamination', 'Protective finish'] },
]

const addOns = ['Pet hair removal', 'Carpet or cloth seat shampoo', 'Leather cleaning and conditioning', 'Headlight restoration', 'Engine bay cleaning', 'Spray ceramic protection', 'Bug and tar removal']
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          {services.map((service, index) => (
            <motion.article key={service.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} viewport={{ once: true }} className="overflow-hidden rounded-2xl border border-white/20 bg-white/5">
              <div className="relative h-52"><img src={service.image} alt={service.title} className="h-full w-full object-cover" /></div>
              <div className="p-6"><h3 className="mb-3 text-2xl font-bold text-white">{service.title}</h3><p className="mb-6 leading-relaxed text-gray-300">{service.description}</p><ul className="space-y-3">{service.items.map((item) => <li key={item} className="flex gap-3 text-sm text-gray-200"><Check className="h-5 w-5 shrink-0 text-blue-400" />{item}</li>)}</ul></div>
            </motion.article>
          ))}
        </div>
